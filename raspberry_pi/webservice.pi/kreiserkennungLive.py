...
class KreisLive():
    def kreislive(cam):
        ...
        kreis_durchmesser_pixel = float(config['conversion']['mm_per_pixel1']) #fragt Wert aus Config File ab
        ...
        if config_test:
            for frame in cam.capture_continuous(rawCapture, format="bgr", use_video_port=True):
                ...
                config['OFFSET'] = {'AbstandZumObjektInCM' : '15',
                                    'DurchmesserKreisInPixel' : kkreis_r,
                                    'naechserkreis_x' : kkreis_xy[0],
                                    'naechsterkreis_y' : kkreis_xy[1],
                                    'geschaetzte distanz' : round((kdistanz*umrechnung_pixel_mm)/10,2)}
                with open('offset.ini', 'w') as configfile:
