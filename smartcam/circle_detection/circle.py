    ...
    def detectCircles(cls, picture=False): #Picture ist bool und ist für das erstellen eines Bildes oder nicht
        ...
        if picture:
            if image_frame is not None:
                ...
                cv2.imwrite("./smartcam/images/" + fileName, image_frame) #speichert es als fileName ab
                logging.debug("saved image: " + fileName + " in: ./smartcam/images/")
                return True
        else:
            if not os.path.exists("./smartcam/images/"):
                try:
                    ...
                    logging.error("Image directory couldn't get created.. \n" + str(e))
            cv2.imwrite("./smartcam/images/circle.jpg", frame) #speichert ein Bild
            return offset
