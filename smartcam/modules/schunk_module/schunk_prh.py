...
class schunk_prh():
	...
	def __init__(self):
		self.dev = serial.Serial('/dev/ttyUSB0', timeout=1)
		self.running = True

	def crc16_add_data(self, crc, data):
		return ((crc & 0xFF00) >> 8) ^ self.tbl[(crc & 0x00FF) ^ (data & 0x00FF)]

	def crc16(self, data):
		crc = 0
		for d in data:
			value = int.from_bytes(d, byteorder='big')
			crc = self.crc16_add_data(crc, value)
		return crc

	def crc16_p(self, data):
		crc = 0
		arr = bytearray(data)
		for b in arr:
			crc = self.crc16_add_data(crc, b)
		return crc

	def write_command(self, id, cmd, data):
		...
		crc = self.crc16_p(pack)
		pack += struct.pack('<H', crc)

		crc = self.crc16_p(pack)
		print('>>> ', pack, crc)
		self.dev.write(bytearray(pack))

	def send_ack(self, id):
		self.write_command(id, 0x8b, [])

	def send_get_state(self, id):
		time = 1.0
		data = list(struct.pack('<f', time))
		data.append(0x07)
		self.write_command(id, 0x95, [])

	def send_fast_stop(self, id):
		self.write_command(id, 0x90, [])

	def send_reference(self, id):
		self.write_command(id, 0x92, [])

	def send_move_to(self, id, pos):
		data = list(struct.pack('<f', pos))
		self.write_command(id, 0xb0, data)

	def send_move_to_s(self, id, pos, speed):
	    data = list(struct.pack('<f', pos))
	    data.extend(list(struct.pack('<f', speed)))
	    self.write_command(id, 0xb0, data)

	def send_get_detailed_error(self, id):
		self.write_command(id, 0x96, [])
	...
	def read_from_port(self):
		...
		while running:
			...
			if len(inp) > 0:
				...
				data.append(inp)
				handled = self.check_message(data)
				data = data[handled:]


	def pos(self, pos, speed=None):
		pos = float(pos)
		if speed is not None:
			speed = float(speed)
			self.send_move_to_s(11, pos, speed)
		else:
			self.send_move_to(11, pos)

	def halt(self):
		self.send_fast_stop(11)

	def ref(self):
		self.send_reference(11)
